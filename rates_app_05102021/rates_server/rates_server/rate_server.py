    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                parsed_client_command = parse_command(client_command_str)

                if not parsed_client_command:
                    self.conn.sendall(b"Invalid Command Format")
                else:
                    self.process_client_command(parsed_client_command)
...
def command_start_server(server_process: Optional[mp.Process],
    ...
    if server_process and server_process.is_alive():
        ...
        server_process = mp.Process(
            target=rate_server, args=(
                host, port, client_count))
